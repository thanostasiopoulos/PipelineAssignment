...
def read_raw_file(
    ...
    path: str,
    mode: str = "PERMISSIVE",
    corrupt_record_col: str = "corrupted_record",
) -> DataFrame:
    df = (
...
def clean_raw_file(df_raw: DataFrame) -> Tuple[DataFrame, Dict[str, int]]:
    ...
    df = df.withColumn(
        "user_id",
        F.when(
            (F.col("user_id") == "") | F.col("user_id").isNull(), F.lit(None)
    ...
    df = df.withColumn(
        "latency_ms",
        F.when(F.col("latency_ms") < 0, F.lit(None)).otherwise(F.col("latency_ms")),
    )
    ...
    df = df.withColumn(
        "status_code",
        F.when(F.col("status_code") < 100, F.lit(None)).otherwise(F.col("status_code")),
    )
    ...
    error_cases["missing_service"] = df.filter(
        missing_service & ~missing_event_id
    ).count()
    error_cases["missing_event_type"] = df.filter(
        missing_event_type & ~missing_event_id & ~missing_service
    ).count()
    error_cases["invalid_timestamp"] = df.filter(
        invalid_timestamp & ~missing_event_id & ~missing_service & ~missing_event_type
    ).count()

    keep_mask = ~(
        missing_event_id | missing_service | missing_event_type | invalid_timestamp
    )
...
def aggregate_metrics(df_clean: DataFrame) -> DataFrame:
    is_error = F.col("status_code").isNotNull() & ~F.col("status_code").between(
        200, 299
    )

    df_grouped = (
        df_clean.groupBy("event_ts", "service")
        .agg(
            F.count("*").alias("request_count"),
            F.avg(F.col("latency_ms")).alias("avg_latency_ms"),
            F.count(F.col("status_code")).alias("status_count"),
            F.try_divide(
                F.sum(F.when(is_error, 1).otherwise(0)).cast("double"),
                F.count(F.col("status_code")),
            ).alias("error_rate"),
        )
